...
typedef struct {
	char rname[5];      /* residue' name */
	char chain[2];      /* residues' chain name */
	int  resnum;        /* residues' sequence number */
	double c_tot_abs;   /* residues' absolute solvent accessable surface area of complex residues */
	double m_tot_abs;   /* residues' absolute solvent accessable surface area of monomer residues */
	double delta;       /* difference between c_tot_abs and m_tot_abs */
	double per;         /* c_tot_abs/m_tot_abs */
	double pos_per;     /* per position of all pers */
} ResCorerim;
